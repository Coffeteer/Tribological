...
Use code chunk below if beginning database. Adds name and starting dataframes
...
tribometer_names <- c("un", "uf", "ud", "um", "ul", "pe", "po", "experiment")
...
tribometer_df <- data.frame(matrix(ncol = 8, nrow = 0))

```

Reads in all properly formatting Mat Files in designated File.
...
colnames(tribometer_df) <- tribometer_names
```
...
saveRDS(averagePlusStd_df, file = paste0(output_file, "averagePlusStd_df.rds"))
saveRDS(overview_df, file =paste0(output_file, "overview_df.rds"))
saveRDS(data_df, file = paste0(output_file, "data_df.rds"))
saveRDS(tribometer_df, file = paste0(output_file, "tribometer_df.rds"))
```
