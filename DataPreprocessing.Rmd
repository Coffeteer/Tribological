...
tribometer_names <- c("un", "uf", "ud", "um", "ul", "pe", "po")
...
tribometer_df <- data.frame(matrix(ncol = 7, nrow = 0))
...
trib_vec <- sub("Mat2.mat", "", basename(file_names))
tribometer_df$experiment <- trib_vec
```



```{r Save read RDS files}
saveRDS(averagePlusStd_df, file = "./RDS_files_new/averagePlusStd_df.rds")
saveRDS(overview_df, file = "./RDS_files_new/overview_df.rds")
saveRDS(data_df, file = "./RDS_files_new/data_df.rds")
saveRDS(tribometer_df, file = "./RDS_files_new/tribometer_df.rds")
```
